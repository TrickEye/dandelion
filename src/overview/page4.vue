...
export default {
  ...
  data() {
    return {
      items: [
        {index: 0, title: '筚路蓝缕，革命底色', link: 'history1', desc: '作为中阳木耳产业的新兴龙头企业，心言生物科技有限公司在政府的强力支持下，最新建设的木耳生产线已于去年投产，该公司同时经营一亿年产量的菌棒生产以及1000栋规模的大棚种植，利用自动化的生产设备实现高效生产，同时着眼于全产业链的木耳深加工，并注重鱼菜共生的水资源循环利用。希望中阳的木耳产业在政府的扶持下，在心言公司的带领下继续茁壮成长，快速发展。', pic: fungusPic},
        {index: 1, title: '稳步发展，生产复苏', link: 'history2',  desc: '山西中阳钢铁有限公司，坐落于中阳县城北部，始建于1985年，如今已发展成为中阳的第一大企业，年产值150亿元，拥有职工上万名，集采煤、选煤、采矿、选矿、炼铁、炼钢、轧钢、发电于一体，具有庞大的产业规模。厂区内食堂、商店、宿舍等生活设施也一应俱全，具有明显的20世纪大型企业的社会化特征，突出体现了以人为本的企业特色。近年来，中钢与北航进行深度技术合作，研究出了以Q195为代表的一系列高技术钢铁产品。', pic: steelPic},
        {index: 2, title: '乡村振兴，全面脱贫', link: 'history3', desc: '柏洼古井手工醋庄坐落于泊洼山脚下，董事长王先生传承家族的古法手艺，在酿醋领域已经深耕了三十多年，通过使用小麦、柏叶、松针等不同的原料，开发了松针醋、柏叶醋、高粱醋等高端保健醋、养生醋等不同的商品类型，带领柏洼手工醋走上多元化、高端化、高附加值的发展道路。', pic: vinegarPic},
        // {index: 3, title: '孵化园', link: '#/overview/', desc: '吕梁市北航中汇科技孵化器，为北航与当地政府合作召集、培养新兴产业的基地，内含硕为思集团、华艺民俗文化发展有限公司、山西宸宇服装有限公司等新兴产业公司，分别代表了在传统重工业以外的网络技术、传统文化、轻工业，为中阳县的多元化发展与产业转型提供了道路指引，为这个曾经的贫困县带来了发展潜力与生产活力。', pic: incubationPic},
      ],


      selectedEntry: 0,
      p4: document.querySelector('#p4'),
      cards: document.querySelectorAll('.history-list .v-card'),
      picShowcases: document.querySelectorAll('.history .pic-showcase'),
    }
  },
  methods: {
    playEntryAnim() {
      document.querySelectorAll('#p4').forEach((col) => {
        col.classList.add('anim')
      })

      document.querySelectorAll('.history-list .v-card').forEach((col, index) => {
        setTimeout(() => {
          col.classList.add('card-entry-animation')
        }, 300 * index)
      })

      document.querySelectorAll('.history .v-expansion-panels').forEach((el, index) => {
        setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        this.cards = document.querySelectorAll('.history-list .v-card');
        this.cards[this.selectedEntry].classList.add('selected');
        this.picShowcases = document.querySelectorAll('.history .pic-showcase');
        this.picShowcases[this.selectedEntry].classList.add('selected');
      }, 3000);
    },
    visible() {
      const rect = p4.getBoundingClientRect(); // I actually don't know why here `this.p2` does not work, while `p2` works, maybe because in vue, p2 can be used to refer the element#p2.
...
<template>
  <v-col cols="12" class="pa-0 ma-0 history">
          ...
          <v-row>
            <v-col cols="12">
              <div class="pr-10 pt-6 text-light-blue">
                <h2 class="text-end" id="p4">历史印记</h2>
              </div>
            </v-col>
              ...
              <v-container>
                <v-row class="history-list">
                  <v-col cols="12" v-for="item in items">
                    <v-card class="bg-indigo mx-auto h0w0 shrink-on-hover d-flex align-center my-5" :ripple="false" @click="() => switchChecked(item.index)">
